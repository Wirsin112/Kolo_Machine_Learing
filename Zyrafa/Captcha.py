...
class Captcha(QtWidgets.QMainWindow):
    ...
    def save(self):
        ...
        if self.rmsdiff(cv2.imread("siema.png"), cv2.imread("show.png")) and self.model(
                cv2.imread("siema.png", cv2.IMREAD_GRAYSCALE)):
            self.label.pixmap().save("data/" + str(time.time()) + ".png", "PNG")
            self.label.pixmap().fill(QtGui.QColor("white"))
            self.label.hide()
            self.label.show()
            print("Brawo narysowales zyrafe")
        else:
            self.label.pixmap().save("data2/" + str(time.time()) + ".png", "PNG")
            self.label.pixmap().fill(QtGui.QColor("white"))
            self.label.hide()
            self.label.show()
        ...
        for i in range(200):
            for j in range(200):
                ...
                if image1[i][j][0] == image2[i][j][0] and image1[i][j][1] == image2[i][j][1] and image1[i][j][2] == \
                    ...
                    good2 += 1
                allw += 1
        if good / all > 0 and good2 / allw > 0.6:
            print(good / all)
            return True
        else:
            print(good / all)
    ...
    def model(self, image1):
        ...
        print("W ilu procentach jest to zyrafa", preds)
        if preds > 0.5:
            return True
        else:
            return False

    def save_tests_to_model(self):
